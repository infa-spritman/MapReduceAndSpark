...
//        ranks = ranks :+ (i + j, intialPR)
      }
      graph = graph :+ (i + k - 1, 0)
//      ranks = ranks :+ (i + k - 1, intialPR)
    }
...
//    ranks.collect().foreach(println)
    ...
    for (iterationCount <- 1 to k*k +1 ) {
      ...
      ranks = temp2.map(x => if(x._1 != 0) (x._1, delta/volume + x._2) else (x._1, 0.0))

      for {i <- 1 to k * k
           if i % k == 1 || k == 1} {
        ranks = ranks.union(sc.parallelize(Seq((i, delta/volume))))
      }
    }
